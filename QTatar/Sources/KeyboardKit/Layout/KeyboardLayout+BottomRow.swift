...
public extension KeyboardLayout {
    ...
    /// Get the system action items at the top row.
    var topRowSystemItems: [KeyboardLayout.Item] {
    ...
    /// Try to create a bottom row item.
    ///
    /// This uses the first ``topRowSystemItems`` item as
    /// template, if you don't provide an action. If none is
    /// found, the function returns `nil`.
    func tryCreateRowItem(
        for action: KeyboardAction
    ) -> KeyboardLayout.Item? {
        guard let template = topRowSystemItems.first else { return nil }
        return KeyboardLayout.Item(
            action: action,
            size: template.size,
            edgeInsets: template.edgeInsets
